...
$(document).ready(function () {
  var $card = $(".card"),
    ...
    $icon = $(".icon");
  let cartPageBottom = {};
  const cart_page = ["title", "text", "text_2"];
  cart_page.forEach((element) => {
    cartPageBottom[element] = document.querySelector(
      `.cart-page-bottom .${element}`
    );
  });

  let text = {
    title: "Hello em ❤️!",
    text: "Chúc em yêu một mùa Giáng sinh an lành, hạnh phúc và ngập tràn niềm vui. Phải luôn khỏe mạnh, xinh đẹp và luôn luôn tỏa sáng như vì sao trên trời :>>> . Yêu em rất nhiều ❤️ my bebi ",
    text_2: "Đây là một món quà nho nhỏ anh dành tặng cho em ❤️",
  };
  let charArr = {
    title: text.title.split(""),
    content: text.text.split(""),
    content_2: text.text_2.split(""),
  };
  function resetText() {
    cart_page.forEach((element) => {
      cartPageBottom[element].innerHTML = "";
    });
  }
  $card.on("click", function () {
    $(this).toggleClass("is-opened");
    if ($card.hasClass("is-opened")) {
      typeText(cartPageBottom.title, charArr.title, 100, function () {
        typeText(cartPageBottom.text, charArr.content, 100, function () {
          typeText(cartPageBottom.text_2, charArr.content_2, 100);
        });
      });
    } else {
      resetText();
  ...
  function typeText(element, textArray, interval, callback) {
    let currentIndex = 0;
